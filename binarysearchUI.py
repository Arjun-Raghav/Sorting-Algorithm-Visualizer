...
def StartAlgorithm():
    ...
    if algMenu.get() == 'Binary Search':
        ...
        binary_search(data, int(entry_select.get()), drawData, speedScale.get())
...
Label(master=UI_frame, text="Enter the value to search: ", width=35, relief=RAISED, borderwidth=5).grid(row=1, column=4, padx=5, pady=5, sticky=W)
entry_select = ttk.Entry(master=UI_frame, width=10)
entry_select.grid(row=1, column=5, padx=5, pady=5)
